...
class S1_Model(VideoBaseModel):

    def __init__(self, opt):
        super(S1_Model, self).__init__(opt)
        if self.is_train:
    ...
    def optimize_parameters(self, current_iter):
        self.output = self.net_g(self.lq, self.event, self.gt, use_LE=True)
        ...
        # edge loss
        if self.edge_loss:
            l_edge = self.edge_loss(self.output.view(-1, c, h, w), self.gt.view(-1, c, h, w))
            l_total += l_edge
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for i in range(rank, num_folders + num_pad, world_size):
            ...
            val_data['event'].unsqueeze_(0)
            self.feed_data(val_data)
            val_data['lq'].squeeze_(0)
            val_data['gt'].squeeze_(0)
            val_data['event'].squeeze_(0)
    ...
    def test(self, idx):
        ...
        with torch.no_grad():
            self.output = self.net_g(self.lq, self.event, self.gt, use_LE=True)
            # feat = self.net_g.feat
            # feat = np.squeeze(feat, axis=0)
            # np.save(f'/home/frank/project/VD-Diff-main/Event/{idx}.npy', feat)

            # if 'REBlur' in self.opt['name']:
            #     self.output = self.output[:, :, :, 30:30 + 260, :]
            #     self.gt = self.gt[:, :, :, 30:30 + 260, :]
