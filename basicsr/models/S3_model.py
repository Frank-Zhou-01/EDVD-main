...
class S3_Model(VideoBaseModel):

    def __init__(self, opt):
        super(S3_Model, self).__init__(opt)
        if self.is_train:
        ...
        if self.opt['path'].get('pretrain_network_g', None) is None:
            load_path = self.opt['path'].get('pretrain_network_S2', None)
    ...
    def optimize_parameters(self, current_iter):
        self.output, IPR_S2, IPR_DM = self.net_g(self.lq, self.event, self.gt)
    ...
    def test(self):
        self.net_g.eval()
        with torch.no_grad():
            self.output = self.net_g(self.lq, self.event, y=None)
