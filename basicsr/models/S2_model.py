...
class S2_Model(VideoBaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.opt['path'].get('pretrain_network_g', None) is None:
            ...
            if load_path is not None:
                ...
                self.load_network(self.net_g.module.DeblurNet, load_path, True, param_key)
    ...
    def optimize_parameters(self, current_iter):
        IPR_S1, IPR_DM = self.net_g(self.lq, self.event, self.gt)
        # print(IPR_S1.shape, IPR_DM.shape)
    ...
    def test(self):
        self.net_g.eval()
        with torch.no_grad():
            self.output = self.net_g(self.lq, self.event, None)
