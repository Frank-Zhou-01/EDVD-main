...
class EdgeLoss(nn.Module):
    def __init__(self, loss_weight=1.0):
        ...
        # 定义单通道拉普拉斯核
        single_kernel = torch.tensor([[0, 1, 0],
                                      [1, -4, 1],
                                      [0, 1, 0]], dtype=torch.float32)

        # 扩展为3通道核 (3x3x3)
        # 方法1：每个通道使用相同的拉普拉斯核
        self.kernel = single_kernel.view(1, 1, 3, 3).repeat(3, 1, 1, 1)
        # 现在形状是 [out_channels=3, in_channels=1, 3, 3]
    ...
    def forward(self, pred, target):
        ...
        # 输入形状: [N, 3, H, W]
        # 对每个通道独立应用相同的拉普拉斯核
        pred_edge = F.conv2d(pred, self.kernel, padding=1, groups=3).abs()
        target_edge = F.conv2d(target, self.kernel, padding=1, groups=3).abs()
